...
final class RandomUtilsTest extends FunTestSuite {
  ...
  test("generating random positive Int with upper bound") {
    val randomInt = RandomUtils.randomPositiveInt(upperBound = 5)
    assert(randomInt >= 1)
    assert(randomInt <= 5)
  }

  test("generating random positive Long with upper bound") {
    val randomLong = RandomUtils.randomPositiveLong(upperBound = 10L)
    assert(randomLong >= 1L)
    assert(randomLong <= 10L)
  }
}
